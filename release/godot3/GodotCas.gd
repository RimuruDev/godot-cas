...
# Returns true if any active network runs over a VPN transport.
# Ad revenue is geo-dependent, so a VPN can skew fill rate and eCPM — use this to
# detect the case rather than to block ads.
func isVpnActive() -> bool:
	return _cas.isVpnActive()
